...
Dokumentace všech tříd a metod, které můžete použít je dostupná na [bang.honzaa.cz/apidocs](https://s.honzaa.cz/bang-sdk-javadoc).
...
1.  **Vytvořte si Maven projekt.**
    * Budete k tomu potřebovat IDE (např. Netbeans).

2.  **Přidejte `plugin-sdk` jako závislost.**
    * Stáhněte si `plugin-sdk` buď celé z repozitáře, nebo použijte [připravený **.jar** soubor](https://s.honzaa.cz/bang-sdk.jar).
...
7.  **Vytvořte postavy a role.**
    * Role jsou jednoduché označení a postavy mohou mít svoje efekty.
