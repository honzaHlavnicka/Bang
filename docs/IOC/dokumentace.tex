...
Opravdové načítání pluginů provádí třída \texttt{NacitacPluginu}. Ta v metodě \texttt{nactiPluginy(Path cestaKeSlozce)} otevře požadovanou složku a pomocí \texttt{DirectoryStream} najde všechny soubory s příponou \texttt{.jar}. Pro každý takový soubor zavolá metodu \texttt{nactiPluginyZJARu(Path cestaKJARu)}, která pomocí \texttt{URLClassLoader} načte třídy z JARu a pomocí \texttt{Enumeration} hledá všechny třídy (soubory s koncovkou \texttt{.class}). Tyto třídy si načte \texttt{ClassLoader}em a ověří, zda jsou přiřaditelné k \texttt{HerniPlugin}, nejsou abstraktní a nejsou rozhraní. Pokud třída splňuje tyto podmínky, tak se vytvoří její instance a přidá do seznamu pluginů. Pokusy o načítání tříd jsou obalené v try bloku, jelikož je v nich mnoho potenciálních chyb, které je potřeba zachytit, aby se načítání pluginů nezastavilo kvůli jednomu špatnému pluginu, například když jeho konstruktor má nějaký parametr, či když nedpovídá stejné versi \emph{Javy} jako server.
...
Přidej život je podobná metoda s opačným účinkem. Míso smrti kontroluje, zda se nepřesáhne daný maximum životů, ktré hráč může mít.

\subsubsection{\texttt{odehranaKarta(String id)}, \texttt{vyloitKartu(String id)} a \texttt{spalitKartu(String id, String idHrace)}}
Tuto metodu volá komunikátor hry, když se hráč rozhodne zahrát kartu. Není určená pro plugin, protože obsahuje všechny kontroly pro vstup od uživatele, navíjako parametr bere \emph{String} id karty a ne samotný objekt, což je v rámci serveru neefektivní.

Metoda si převede id do \emph{int}u, ověří, že je hráč na tahu a pokusí se kartu najít mezi kartami v ruce porovnáváním id. Poté zkontroluje, zda je karta hratelná (instance \texttt{HratelnaKarta}), přetypuje ji, zeptá se pravidel jestli ji může zahrát a pokud ano, tak zavolá její metodu \texttt{odehrat(Hrac kdo)}. Pokud se karta zahrála, tak ji přesune na odhazovací balíček, provede efekty a upozorní komunikátor hry, který informuje klienty o změně stavu hry. V tomto procesu nastalo spoustu chybových stavů, které vždy předají konkrétní chybu komunikátoru, který ji zobrazí klientovi. 

Podobným způsobem funguje také spálení a vyložení karty, které zajišťují metody \texttt{vyloitKartu(String id)} a \texttt{spalitKartu(String id, String idHrace)}.
...
Serverová část projektu se buildí pomocí \emph{Maven}u. Build není potřeba zařizovat přímo. Stačí použit připravený script \texttt{build.sh}, který se nachází v kořenovém adresáři projektu. Tento script vytvoří složku \texttt{build}, ve které se nachází jak zkompilovaný server, tak složka s pluginy a script \texttt{start.sh}, který spustí server. Pro build klienta stačí použít \texttt{npm run build} ve složce \texttt{klient\_react/bang}, což vytvoří složku \texttt{dist} s připravenými statickými soubory pro server.
...
Pro testování existuje i několik nástrojů, například pro zobrazení boxů kolem všech prvků  DOM stačí přidat body třídu \texttt{testovani}, nebo po zobrazení celé websocketové komunikace stačí otevřít konzoli prohlížeče a podívat se na zelené zprávy. Také existuje pomůcka \texttt{serverInfo.html}, která zobrazuje informace o serveru, které jsou užitečné pro testování a správu serveru a \texttt{gameInfo.html}, která zobrazuje informace o libovolé hře, které jsou užitečné pro testování a ladění pluginů, pokud je například chyba s neshodou mezi reálným stavem hry a stavem hry, který si vykládá klient.


\section{Budoucí rozšiřování}
Projekt je navržený tak, aby se dal snadno rozšiřovat. Díky pluginům je možné přidat další hry, které budou fungovat na stejném enginu. V plánu je přidat některá rozšíření pro \emph{Bang!}, které již jsou předpřipravené. Z nových her jsou již promyšlené výbušná koťátka a kvarteto, která využijí některé funkce enginu, které se ještě nevyužívají. Dále by bylo možné přidat i další funkce pro klienta, například možnost přizpůsobení vzhledu, přidání zvuků a podobně. Pro klienta je v plánu přidat peer-to-peer voice chat, který pomůže udělat hru akčnější a zábavnějšší. Tento chat bude fungovat pomocí WebRTC, které umožňuje přímou komunikaci mezi prohlížeči bez potřeby serveru pro přenos dat. Server bude sloužit jako zprostředkovatel přenosu připojovacách tokenů z knihovny \emph{peerJS}.
...
\section{Přílohy}
V odevzdávacím systému vyuka.gyarab.cz je přiložený repozitář projektu ke stavu k datu odevzádní projektu.
\section{Zdroje}
