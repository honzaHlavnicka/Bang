...
Komunikátor hry každému připojenému hráči přiřazuje token ve formátu \texttt{kód hry + náhodný řetězec}, který slouží k identifikaci hráče po znovupřipojení. Pokud se hráč chce znovu připojit, tak klient pošle kód, který si uložil do localStorage, a komunikátor ho podle něj identifikuje a spojí ho s jeho hráčem. Ke každému hráči může být pouze jedno spojení, takže pokud se hráč pokusí připojit s tokenem, který už je připojený, tak se ten původní odpojí a nahradí ho ten nový. To řeší situaci, kdy uživatel nechal starou hru otevřenou a zapoměl na to kde.

Komunikátor si hlídá hráče, které se odpojili a pokud se odpojí všichni hráči a nepřipojí se do určitého časového limitu, tak hru smaže, aby se nezdržovala v paměti. Tento časový limit je nastaven na 5 minut, což by mělo být dostatečné pro většinu případů, kdy hráč odpojí kvůli technickým problémům.
...
Projekt obsahuje možnost vrácení se do hry po nenadálém odpojení při výpadku sítě, nebo třeba zavření okna. Pro tento účel musel být zaveden znovupřipojovací token, který je při připojení se k hře vygenerován a poslán klientovi. Tento token je pak uložen v \texttt{localStorage} a při načítání stránky se zkontroluje, jestli tam není. Pokud tam je, tak se uživatelovi nabídne možnost se vrátit do hry, což po potvrzení pošle serveru token a id hráče, aby ho mohl znovu připojit ke hře. Kvůli této funkcionalitě nastal problém se synchronizací stavu, kdy se při připojení musí načíst úplně všechna data. Nejdříve spousta dat chybělo, ale postupně se přidávaly další a další, až se nakonec podařilo načítat všechna data, která jsou potřeba pro správné zobrazení stavu hry.
